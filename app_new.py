...
def get_gallery_images(tone=None):
    ...
    print(f"Looking for images in: {upload_path}")
    ...
    for ext in ['*.jpg', '*.jpeg', '*.png', '*.gif']:
        ...
        print(f"Found {len(all_images)} images for {ext}")
        ...
        if tone:
            ...
            print(f"After filtering for {prefix}: {len(all_images)} images")
        
        images.extend(all_images)
    
    result = sorted([os.path.basename(img) for img in images])
    print(f"Final images for tone {tone}: {result}")
    return result
...
@app.route('/gallery')
def gallery():
    romantic_images = get_gallery_images('romantic')
    playful_images = get_gallery_images('playful')
    return render_template('gallery.html', 
                         romantic_images=romantic_images,
                         playful_images=playful_images)
...
def index():
    ...
    print(f"Sending tone_images to template: {tone_images}")
